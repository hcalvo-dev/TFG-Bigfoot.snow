...
export default function ReservarClase({ session }: Props) {
  ...
  const procesarPagoYReservar = (e: React.FormEvent) => {
    ...
    fetch('http://localhost:4000/api/reserva/clase', {
      method: 'POST',
  ...
  const handlePagar = (data: any) => {
    setPantallaCongelada(true); // ← Activar congelación
    setSuccessPagar(true);
    onPay(data);
    setTimeout(() => {
      setSuccessPagar(false);
      setPantallaCongelada(false); // ← Desactivar congelación
      cerrarModal();
    }, 2000);
  };

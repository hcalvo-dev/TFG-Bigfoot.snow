...
const paymentSchema = z.object({
  cardNumber: z
    .string()
    .regex(/^4[0-9]{15}$/, 'Debe ser una tarjeta Visa válida'),
  expiry: z
    .string()
...
export default function ReservarClase({ session }: Props) {
  ...
  const procesarPagoYReservar = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  useEffect(() => {
    if (tarjeta && tarjeta.length === 2 && !tarjeta.includes('/')) {
      setValue('expiry', tarjeta + '/');
    }
  }, [tarjeta, setValue]);

  const onPay = (data: any) => {
    procesarPagoYReservar(new Event('submit') as any);
  };
          ...
          <motion.div
            ...
            <motion.div
              ...
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-green-500 text-white p-4 rounded-full shadow-lg">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M5 13l4 4L19 7" /></svg>
              </div>

              <h2 className="text-2xl font-extrabold text-center mb-6 text-blue-800 dark:text-white">Resumen de tu reserva</h2>
              <div className="space-y-4 text-gray-800 dark:text-gray-300 text-base px-2">
                <p><span className="font-semibold text-blue-700 dark:text-blue-300">Monitor:</span> {instructorSeleccionado?.usuario.nombre}</p>
                <p><span className="font-semibold text-blue-700 dark:text-blue-300">Fecha:</span> {fechaSeleccionada}</p>
                <p><span className="font-semibold text-blue-700 dark:text-blue-300">Horas:</span> {horasSeleccionadas.join(', ')}</p>
                <p className="text-lg"><span className="font-bold text-green-600 dark:text-green-400">Total:</span> <span className="text-xl font-bold">{precioTotal}€</span></p>
              </div>

              {/* Si el usuario está autenticado, mostrar pago. Si no, redirige al login */}
               {mostrarPago ? (
                <div className="mt-8">
                  <h3 className="text-lg font-semibold mb-4 text-center text-blue-500">Introduce los datos de tu tarjeta</h3>
                  <form className="space-y-4" onSubmit={handlePayment(onPay)}>
                    <div>
                      <input
                        ...
                        maxLength={16}
                        className={`w-full px-4 py-2 rounded border ${paymentErrors.cardNumber ? 'border-red-500' : 'border-gray-300'} dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white`}
                      />
                    ...
                    <div className="flex gap-4">
                      <div className="w-1/2">
                        <input
                          ...
                          placeholder="MM/AA"
                          maxLength={5}
                          className={`w-full px-4 py-2 rounded border ${paymentErrors.expiry ? 'border-red-500' : 'border-gray-300'} dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white`}
                        />
                        {paymentErrors.expiry && <p className="text-red-500 text-sm mt-1">{paymentErrors.expiry.message}</p>}
                      </div>
                      <div className="w-1/2">
                        <input
                          {...registerCard('cvv')}
                          ...
                          placeholder="CVV"
                          maxLength={3}
                          className={`w-full px-4 py-2 rounded border ${paymentErrors.cvv ? 'border-red-500' : 'border-gray-300'} dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white`}
                        />
                        {paymentErrors.cvv && <p className="text-red-500 text-sm mt-1">{paymentErrors.cvv.message}</p>}
                      </div>
                    </div>
                    <div className="flex justify-end gap-4">
                      <button type="button" onClick={() => setMostrarModal(false)} className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white px-5 py-2 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition">Cancelar</button>
                      <button type="submit" className="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700 font-semibold shadow transition">Pagar y Reservar</button>
                    </div>
                  </form>
                </div>
              ) : (

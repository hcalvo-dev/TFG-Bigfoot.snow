...
export default function StickyImageEffect() {
  ...
  const rombosOpacity = useTransform(scrollYProgress, [0.4, 0.49], [0, 1]);
  const firstMobileBlockOpacity = useTransform(scrollYProgress, [0.3, 0.4], [1, 0]);
  const firstMobileBlockY = useTransform(scrollYProgress, [0.3, 0.4], [0, -100]);
  ...
  return (
    <section ref={containerRef} className="relative w-full min-h-[400vh]">
      ...
      <DesktopContent
        ...
        logosOpacity={logosOpacity}
        iconsOpacity={iconsOpacity}
        rombosOpacity={rombosOpacity}
      />
      <MobileContent
